...
class TranslateVisitor(IRVisitor):
    ...
    def visit_block(self, element: Block) -> translate.Exp:
        exp: tree.Stm = tree.CONST(0)

        if element.statement_list.size() != 0:
            exp = tree.EXP(element.statement_list.element_at(0).accept_ir(self).un_ex())
            for i in range(1, element.statement_list.size()):
                exp = tree.SEQ(exp, tree.EXP(element.statement_list.element_at(i).accept_ir(self).un_ex()))

        return translate.Exp(tree.ESEQ(exp, tree.CONST(0)))

    
    def visit_if(self, element: If) -> translate.Exp:
        condition_exp : translate.Exp = element.condition_exp.accept_ir(self)
        if_exp : translate.Exp = element.if_statement.accept_ir(self)
        else_exp : translate.Exp = element.else_statement.accept_ir(self)

        label_true : tree.LABEL = tree.LABEL(temp.Label())
        label_false : tree.LABEL = tree.LABEL(temp.Label())
        label_done: tree.LABEL = tree.LABEL(temp.Label())
                                
        condition: tree.CJUMP = tree.CJUMP(tree.CJUMP.EQ, condition_exp.un_ex(), tree.CONST(1), label_true.label, label_false.label)

        if_stm: tree.SEQ = tree.SEQ(tree.SEQ(label_true, if_exp.un_ex()) , tree.JUMP(label_done.label))
        else_stm: tree.SEQ = tree.SEQ(tree.SEQ(label_false, else_exp.un_ex()) , tree.JUMP(label_done.label))


        translate_exp:  translate.Exp = translate.Exp(tree.ESEQ(tree.SEQ(condition, tree.SEQ(if_stm, tree.SEQ(else_stm, label_done)))), tree.CONST(0))

        return translate_exp
    ...
    def visit_while(self, element: While) -> translate.Exp:
        ...
        not_condition_exp: tree.BINOP = tree.BINOP(tree.BINOP.XOR, tree.CONST(1), condition_exp.un_ex())

        condition: tree.CJUMP = tree.CJUMP(tree.CJUMP.EQ, not_condition_exp, tree.CONST(1), done.label, body_label.label)
                                            #se a condição(negada) for igual a 1, entao vou para done
        test_condition: tree.SEQ = tree.SEQ(test, condition)
        body_stm: tree.SEQ = tree.SEQ(body_label, tree.SEQ(stm.un_ex(), tree.SEQ(tree.JUMP(test.label), done)))

        return translate.Exp(tree.ESEQ(tree.SEQ(test_condition, body_stm), tree.CONST(0)))
    ...
    def visit_array_assign(self, element: ArrayAssign) -> translate.Exp:
        ...
        array_name: translate.Exp = translate.Exp(element.array_name_id.accept_ir(self))
        array_exp: translate.Exp = translate.Exp(element.array_exp.accept_ir(self))
        right_exp: translate.Exp = translate.Exp(element.right_side_exp.accept_ir(self))

        move: tree.MEM = tree.MEM(tree.BINOP(tree.BINOP.PLUS,array_name.un_ex(), tree.BINOP(tree.BINOP.MUL, tree.BINOP(tree.BINOP.PLUS, array_exp.un_ex(), tree.CONST(1)), tree.CONST(self.current_frame.word_size))))
        assign: tree.MOVE = tree.MOVE(move, right_exp.un_ex())
    ...
    def visit_less_than(self, element: LessThan) -> translate.Exp:
        ...
        return translate.Exp(tree.BINOP(tree.BINOP.LSHIFT, left.un_ex(), right.un_ex()))
    ...
    def visit_plus(self, element: Plus) -> translate.Exp:
        ...
        return translate.Exp(tree.BINOP(tree.BINOP.PLUS, left.un_ex(), right.un_ex()))
    ...
    def visit_minus(self, element: Minus) -> translate.Exp:
        ...
        return translate.Exp(tree.BINOP(tree.BINOP.MINUS, left.un_ex(), right.un_ex()))      
    ...
    def visit_times(self, element: Times) -> translate.Exp:
        ...
        return translate.Exp(tree.BINOP(tree.BINOP.MUL, left.un_ex(), right.un_ex()))
    ...
    def visit_call(self, element: Call) -> translate.Exp:
        calle_exp : tree.EXP = tree.EXP(element.callee_exp.accept_ir(self).un_ex())
        
        arg_list = tree.ExpList()

        for index in range(element.arg_list.size()):
            arg: translate.Exp = element.arg_list.element_at(index).accept_ir(self)
            arg_list.add_tail(arg)
    ...
    def visit_new_array(self, element: NewArray) -> translate.Exp:
        exp: translate.Exp = element.new_exp.accept_ir(self).un_ex
        array = List[tree.Exp]
